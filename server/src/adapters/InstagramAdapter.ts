...
export class InstagramAdapter implements SocialPlatformAdapter {
  ...
  getCapabilities(): PlatformCapabilities {
    return {
      ...
      supportsTitle: false, // Instagram uses caption only
      supportsDescription: false,
      supportsTags: false,
      supportsCarousel: true,
      maxFileSizeMB: 100, // Instagram Graph API limit for video reels
  ...
  async publish(params: PublishMediaParams): Promise<PublishResult> {
    ...
    // Check if real API token is configured in credentials or environment
    ...
    try {
      // In production with real Meta Graph token:
      // 1. POST https://graph.facebook.com/v19.0/{ig-user-id}/media (create media container)
      // 2. POST https://graph.facebook.com/v19.0/{ig-user-id}/media_publish (publish container)
