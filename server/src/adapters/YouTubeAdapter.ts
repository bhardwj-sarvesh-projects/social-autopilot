...
export class YouTubeAdapter implements SocialPlatformAdapter {
  ...
  getCapabilities(): PlatformCapabilities {
    return {
      ...
      maxFileSizeMB: 256000, // 256 GB for verified accounts
      supportedFormats: ['video/mp4', 'video/quicktime', 'video/webm', 'video/x-matroska'],
      aspectRatios: ['16:9', '9:16', '1:1', '4:3'],
      maxCaptionLength: 5000, // Description length
  ...
  async publish(params: PublishMediaParams): Promise<PublishResult> {
    ...
    try {
      // Build hashtags into description if provided
      ...
      // If this content has a driveFileId, stream upload via youtubeService & Google auth
      if (content.sourceId) {
        try {
      ...
      // Fallback: simulated YouTube upload
